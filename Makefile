...
ip-test:
	docker run --rm --network=container:qbittorrent alpine:3.20 sh -c "apk add wget && wget -qO- https://ipinfo.io"

speedtest:
	docker run --rm --network=container:qbittorrent ubuntu:22.04 sh -c "apt-get update && apt-get install curl -y && curl -s https://packagecloud.io/install/repositories/ookla/speedtest-cli/script.deb.sh | bash && apt-get update && apt-get install speedtest && speedtest --accept-gdpr --accept-license"

dns-leak-test:
	docker run --rm --network=container:qbittorrent ubuntu:22.04 sh -c "apt-get update && apt-get install curl inetutils-ping -y && curl https://raw.githubusercontent.com/macvk/dnsleaktest/master/dnsleaktest.sh -o dnsleaktest.sh && chmod +x dnsleaktest.sh && ./dnsleaktest.sh"
